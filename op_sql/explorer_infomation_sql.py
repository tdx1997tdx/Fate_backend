...
def explorer_infomation(id,conn_info=c.get_now_conn()):
    ...
    dic_info = {}
    big_sql = "select region_name,origin_name,prototype_name,r.region_id,o.origin_id,p.prototype_id from servent_and_prototype sp inner join prototype p on sp.prototype_id=p.prototype_id inner join prototype_and_origin po on sp.prototype_id=po.prototype_id inner join prototype_and_region pr on sp.prototype_id=pr.prototype_id inner join region r on pr.region_id=r.region_id inner join origin o on po.origin_id=o.origin_id where servent_id=%s"
